...
import dalex as dx
def SVMOriginal(bin_dataset_train,bin_dataset_test,arr, X_train, X_test, y_train, y_test ,A_train, A_test, sensitive_param, program_name = "", dataset_name = "", search_name = "", save_model=False, start_time = 0, index = None):
    try:
...
#------------------------------
def logisticRegressionOriginal(arr, X_train, X_test, y_train, y_test ,A_train, A_test, sensitive_param, save_model=False, index = None):
    try:
        ...
        clf3 = LogisticRegression(penalty=arr[1], dual = arr[2], tol = arr[3],
                C = arr[4], fit_intercept = arr[5], intercept_scaling = arr[6],
                solver=arr[0], max_iter = arr[7], multi_class=arr[8], l1_ratio = arr[9],
                random_state=arr[11], class_weight = arr[10], verbose = arr[12],
                warm_start = arr[13], n_jobs=arr[14])
    ...
    return True, clf3, arr, score, pred 
def lassoRegressionOriginal(arr, X_train, X_test, y_train, y_test ,A_train, A_test, sensitive_param, save_model=False, index = None):
    try:
        ...
        clf3 = Lasso(alpha=arr[0], fit_intercept=arr[1], precompute=arr[2], copy_X=arr[3], max_iter=arr[4], 
                      tol=arr[5], warm_start=arr[6], positive=arr[7], random_state=arr[8], selection=arr[9])
    except ValueError as ve:
        print('failed input',arr) 
        print("here you go------------------------------------")
    ...
    return True, clf3, arr, score, pred,clf2
