...
def get_counts(df, y_pred, biased_col, metric, yname, priv, unpriv):
    ...
    test_df_copy['TP_' + biased_col + "_1"] = np.where((test_df_copy[yname] == 1) &
                                           ...
                                           (test_df_copy[biased_col] == priv), 1, 0)
    ...
    test_df_copy['TP_' + biased_col + "_0"] = np.where((test_df_copy[yname] == 1) &
                                                  (test_df_copy['y_pred' + biased_col] == 1) &
                                                  (test_df_copy[biased_col] == unpriv), 1, 0)
...
def situation(clf,X_train,y_train,keyword):
    X_train[keyword] = X_train[keyword].astype(float).astype(int)
    X_flip = X_train.copy()
    X_flip[keyword] = np.where(X_flip[keyword]==1, 0, 1)
...
def measure_final_score(test_df, y_pred, biased_col, metric, yname,priv, unpriv):
    df = copy.deepcopy(test_df)
    return get_counts(df, y_pred, biased_col, metric, yname,priv, unpriv)

def getMetrics(test_df, y_pred, biased_col, treatment, samples, yname, rep, learner, start, clf = None, priv=1, unpriv=0):

    recall = measure_final_score(test_df, y_pred, biased_col, 'recall', yname,priv, unpriv)
    precision = measure_final_score(test_df, y_pred, biased_col, 'precision', yname,priv, unpriv)
    accuracy = measure_final_score(test_df, y_pred, biased_col, 'accuracy', yname,priv, unpriv)
    F1 = measure_final_score(test_df, y_pred, biased_col, 'F1', yname,priv, unpriv)
    AOD = measure_final_score(test_df, y_pred, biased_col, 'aod', yname,priv, unpriv)
    EOD =measure_final_score(test_df, y_pred, biased_col, 'eod', yname,priv, unpriv)
    SPD = measure_final_score(test_df, y_pred, biased_col, 'SPD', yname,priv, unpriv)
    FA0 = measure_final_score(test_df, y_pred, biased_col, 'FA0', yname,priv, unpriv)
    FA1 = measure_final_score(test_df, y_pred, biased_col, 'FA1', yname,priv, unpriv)
    DI = measure_final_score(test_df, y_pred, biased_col, 'DI', yname,priv, unpriv)
    ...
    if clf :
        Flip = round(calculate_flip(clf, test_df[test_df.columns[:-1]],biased_col),2)
    else:
        Flip = None
