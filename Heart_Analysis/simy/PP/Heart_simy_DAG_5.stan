...
for(ind in 1:N){slope[ind] ~ poisson(exp((oldpeakslope * oldpeak[ind])  + slope0));
label[ind] ~ bernoulli_logit((oldpeaklabel * oldpeak[ind])  + label0);
cp[ind] ~ poisson(exp((labelcp * label[ind])  + cp0));
