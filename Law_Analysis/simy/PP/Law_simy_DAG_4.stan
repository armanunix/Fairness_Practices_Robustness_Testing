...
for(ind in 1:N){first_pf[ind] ~ bernoulli_logit((racefirst_pf * race[ind])  + first_pf0);
LSAT[ind] ~ normal((raceLSAT * race[ind])  +(first_pfLSAT * first_pf[ind])  + LSAT0, sigma_h);
sex[ind] ~ bernoulli_logit((racesex * race[ind])  +(LSATsex * LSAT[ind])  + sex0);
