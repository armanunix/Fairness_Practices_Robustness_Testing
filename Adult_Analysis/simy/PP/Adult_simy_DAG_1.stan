...
for(ind in 1:N){e[ind] ~ poisson(exp((oe * o[ind])  +(ne * n[ind])  + e0));
gender[ind] ~ bernoulli_logit((ogender * o[ind])  + gender0);
r[ind] ~ poisson(exp((er * e[ind])  +(or * o[ind])  +(genderr * gender[ind])  + r0));
hr[ind] ~ normal((ehr * e[ind])  +(ohr * o[ind])  +(rhr * r[ind])  +(genderhr * gender[ind])  + hr0, sigma_h);
