...
for(ind in 1:N){y[ind] ~ bernoulli_logit((ey * e[ind])  + y0);
...
m[ind] ~ poisson(exp((agem * age[ind])  +(wm * w[ind])  +(em * e[ind])  +(om * o[ind])  +(rm * r[ind])  +(racem * race[ind])  +(genderm * gender[ind])  +(hrm * hr[ind])  +(ym * y[ind])  + m0));
