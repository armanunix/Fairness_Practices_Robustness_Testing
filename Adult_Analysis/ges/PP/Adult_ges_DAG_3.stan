...
for(ind in 1:N){w[ind] ~ poisson(exp((genderw * gender[ind])  + w0));
e[ind] ~ poisson(exp((we * w[ind])  +(ne * n[ind])  + e0));
r[ind] ~ poisson(exp((wr * w[ind])  +(er * e[ind])  +(genderr * gender[ind])  + r0));
hr[ind] ~ normal((whr * w[ind])  +(ehr * e[ind])  +(rhr * r[ind])  +(genderhr * gender[ind])  + hr0, sigma_h);
