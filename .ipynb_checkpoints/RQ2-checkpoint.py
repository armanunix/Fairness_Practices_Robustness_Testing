...
def generate_dataset_I(data, graph, edges, ave_dist, centroids):
    ...
    for Algorithm in ['pc','ges','simy']:
        ...
        for edge_list_filename in glob.glob('./'+dataset+'_Analysis/'+Algorithm+'/PP/*.csv'):
            ...
            for i in range(KMean_coef.n_clusters):
                ...
                None_model2 = []
                for j in range(10):
                    final_df, succ_rate = generate_dataset_I(df, graph, edges, ave_dist, KMean.cluster_centers_ )
                    if succ_rate==0.0 :
                        continue
                    ...
                    X_train_new, X_test_new, y_train_new, y_test_new, A_train_new, A_test_new = train_test_split(X2, Y2, A2, random_state=i)
                    if priv_group not in A_test_new or unpriv_group not in A_test_new:
                        print('No sens group')
                        continue
                    model = LogisticRegression()
                    model.fit(X_train_new,y_train_new)
                    preds_None = model.predict(X_test_new)
                    acc_None = accuracy_score(y_test_new,preds_None)
                    f1_None = f1_score(y_test_new,preds_None)
                    AOD_None = round(average_odds_difference(y_test_new, preds_None,prot_attr=A_test_new,priv_group=priv_group ),3)
                    EOD_None = eod(y_test_new, preds_None,sens=A_test_new, priv=priv_group, unpriv=unpriv_group)        
                    ...
                    for transformer in ['SelectKBest', 'SelectFpr','SelectPercentile' ,'drop']:
                        ...
                        AOD = round(average_odds_difference(y_test_new, preds_new,prot_attr=A_test_new,priv_group=priv_group ),3)
                        EOD = eod(y_test_new, preds_new,sens=A_test_new, priv=priv_group, unpriv=unpriv_group)
